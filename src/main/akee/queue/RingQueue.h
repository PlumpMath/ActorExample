...
#ifndef _AKEE_QUEUE_RINGQUEUE_H_
#define _AKEE_QUEUE_RINGQUEUE_H_
...
struct RingQueueHead
{
    ...
    char padding1[AKEE_CACHE_LINE_SIZE - sizeof(uint32_t)];

    volatile uint32_t tail;
    char padding2[AKEE_CACHE_LINE_SIZE - sizeof(uint32_t)];
};
...
public:
    static const size_type  kCapacityCore   = (size_type)AKEE_MAX(utils::round_up_to_pow2<Capacity>::value, 2);
...
public:
    static const size_type  kCapacity   = (size_type)AKEE_MAX(utils::round_up_to_pow2<Capacity>::value, 2);
...
#endif  /* _AKEE_QUEUE_RINGQUEUE_H_ */
